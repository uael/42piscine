...
let newDeck =
  Random.self_init ();
  let l = List.length Card.all in
  let rec build = function
  | 0 -> []
  | i -> (List.nth Card.all (Random.int l))::build (i - 1)
  in build 52

let toStringList = function
| [] -> invalid_arg "invalid operation, you idiot"
| d -> List.map (fun c -> Card.toString c) d

let toStringListVerbose = function
| [] -> invalid_arg "invalid operation, you idiot"
| d -> List.map (fun c -> Card.toStringVerbose c) d

let drawCard = function
| [] -> raise (Failure "you idiot")
| h::t -> (h, t)
